...
export async function isAccountDeployed(
  ...
  const provider = new Provider({
    sequencer: {
      baseUrl: starknetNetwork === "mainnet" ? "https://alpha-mainnet.starknet.io" : "https://alpha4-2.starknet.io",
    },
  });
  try {
    ...
    WatchTowerLogger.error(`Failed to check deployment status for ${accountAddress}: ${error}`);
    return false;
  }
}

export function starknetEcVerify(pubkey: string, messageHash: string, signature: string[]): boolean {
  try {
    ...
    const isValid = key.verify(
      num.toHex(msgHashBigInt),
      { r: num.toHex(r), s: num.toHex(s) }
    );

    return isValid;
  } catch (error) {
    WatchTowerLogger.error(`ECDSA verification failed for pubkey ${pubkey}: ${error}`);
    return false;
  }
}
...
export const verifySignature = async (
  ...
  if (isDeployed) {
    try {
      ...
      if (signatureValid) {
        WatchTowerLogger.info(`Signature verified successfully via isValidSignature for deployed account ${accountAddress}`);
      } else {
        WatchTowerLogger.error(`Signature invalid for deployed account ${accountAddress}: ${result[0]}`);
      }
      return {
        signatureValid,
      ...
      try {
        ...
        if (signatureValid) {
          WatchTowerLogger.info(`Signature verified successfully via is_valid_signature for deployed account ${accountAddress}`);
        } else {
          WatchTowerLogger.error(`Signature invalid for deployed account ${accountAddress}: ${result[0]}`);
        }
    ...
    if (!pubkey) {
      WatchTowerLogger.error(`Cannot verify signature for undeployed account ${accountAddress}: public key not provided`);
    ...
    if (signatureValid) {
      WatchTowerLogger.info(`Signature verified successfully via ECDSA for undeployed account ${accountAddress}`);
    } else {
      WatchTowerLogger.error(`Signature invalid via ECDSA for undeployed account ${accountAddress}`);
    }
    return {
      signatureValid,
